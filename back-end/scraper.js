...
const prof_scraper=async (prof, ischool)=>{
  ...
  const results=[];
  for (result of res) {
    const thisRes=await page.evaluate((el)=>el.textContent, result);
    if (thisRes) {
      results.push([thisRes]);
    }
  }
  ...
  const difRow=wantedRow.replace(/\s/g, '');
  const difficulty=difRow.substring(difRow.length-3, difRow.length);
...
>>>>>>> b49d82d3b42d295eb71208fb2f1a880e1d6bb149
